...
$query = isset($_SESSION["user"]) ? "SELECT email, password FROM users WHERE email ='" . $_SESSION['email'] . "'" :
"SELECT email, password FROM supplier WHERE email ='" . $_SESSION['email'] . "'";
...
if ($new_pass != $new_pass1) {
    ...
    if ($old_pass == $orig_pass) {
        $query = isset($_SESSION["user"]) ? "UPDATE  users SET password = '" . $new_pass . "' WHERE email = '" . $_SESSION['email'] . "'" : 
        "UPDATE  supplier SET password = '" . $new_pass . "' WHERE email = '" . $_SESSION['email'] . "'";
